...
Engine.prototype.initGame = function() {
    ...
    for(i=0;i<this.word_shuffled.length;i++){
        ...
        letter = new Letter(this.game, 60*(i+1), 200, this.word_shuffled[i], color[this.word_shuffled[i]]);
        // Set scale of Sprite
    ...
    this.letterGroup.x = getScreenWidth()/2-(this.word.length*45);
}
...
function listener (sprite, pointer) {
    ...
    if(sprite.y!=400){
        ...
        sprite.x = (this.letterGroup.children.indexOf(sprite)+1)*60;
        sprite.y = 200;
        // Update position to next first index free in the answer group 
        if(x_old/60<this.answerIndex){
            this.answerIndex=x_old/60;
        }
    }
    this.checkWin();
}
...
Engine.prototype.checkWin = function() {
    ...
    for(i=0;i<lg.length;i++){
        if( (this.word_shuffled[i]!=this.word[(lg[i].x/60)-1] && lg[i].y==400) || lg[i].y==200 ) {
            return false;
        }
    }
